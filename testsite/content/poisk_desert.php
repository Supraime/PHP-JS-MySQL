<div class="content">
	...
	<?
		...
		$text = $_POST['query'];
		$res = mysql_query("SELECT * FROM `desert` WHERE `title` LIKE '%$text%' OR `kr_descript` LIKE '%$text%'");
...

{
	if ($ar['id']== null){
		...
		$result = mysql_query("SELECT * FROM `desert` WHERE `title` LIKE '%$text%' OR `kr_descript` LIKE '%$text%'");
				$num_result = mysql_num_rows($result);
		for ($i = 0; $i<$num_result; $i++) {
			$row = mysql_fetch_array($result);
			echo '<a href="desert_full.php?id='. $row["id"].'"><div class="mus_close">
